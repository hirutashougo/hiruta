...
public class Exercise06_04 {
	...
	*/
	public static void main(String[] arg) {
		...
		//要素数の分だけ、インデントに乱数を割り振る処理を繰り返す
		for (int i = 0; i < elementCount; i++) {
			//各インデントに1～10の乱数を代入
			verticalValue[i] = 1 + randomNumber.nextInt(RANDOM_RANGE);
		}
		...
		for (int i = 0; i < RANDOM_RANGE; i++) {
			...
			for (int j = 0; j < elementCount; j++) {
				//インデントに代入された数値が、グラフの縦軸数値よりも小さい場合
				if (verticalValue[j] < (RANDOM_RANGE - i)) {
					//記号「　」を表示
					System.out.print(" ");
				//インデントに代入された数値が、グラフの縦軸数値以上の場合
				} else {
					//記号「＊」を表示
					System.out.print("*");
				}
		...
		for (int i = 0; i < elementCount; i++)	{
		//確認の為の表示
		System.out.println("verticalGraph[" + (i+1) + "] = " + verticalValue[i]);
		}
	}
}