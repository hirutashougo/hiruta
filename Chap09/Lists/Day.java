...
public class Day {
	 ...
	 * 作成日：2024/05/10
	*/
	Day(int yearData, int monthData, int dateData) {
		//年の情報のクラス型変数を初期化
	...
	*/
	int deriveDayOfWeek() {
		...
		//ツェラーの公式で用いる定数その3を宣言
		final int THIRD_ZELLER_CONSTANT = 13;
		//ツェラーの公式で用いる定数その4を宣言
		final int FORTH_ZELLER_CONSTANT = 8;
		//ツェラーの公式で用いる定数その5を宣言
		final int FIFTH_ZELLER_CONSTANT = 5;
		//ツェラーの公式で用いる定数その6を宣言
		final int SIXTH_ZELLER_CONSTANT = 7;
		...
		dayOfweek = (targetYear + (targetYear / FIRST_ZELLER_CONSTANT) - (targetYear / SECONT_ZELLER_CONSTANT) + ((THIRD_ZELLER_CONSTANT * targetMonth + FORTH_ZELLER_CONSTANT) / FIFTH_ZELLER_CONSTANT) + dateData) % SIXTH_ZELLER_CONSTANT;
