...
//shapeパッケージのParallelogramクラスをインポート
...
public class ShapeTester {
	...
	*/
	public static void main(String[] args) {
		...
		//表示する図形として、点を選択したことを表す定数を定義
		final int CHOSE_POINT = 1;
		//表示する図形として、水平直線を選択したことを表す定数を定義
		final int CHOSE_HORIZON_LINE = 2;
		//表示する図形として、垂直直線を選択したことを表す定数を定義
		final int CHOSE_VIRTICLE_LINE = 3;
		//表示する図形として、長方形を選択したことを表す定数を定義
		final int CHOSE_RECTANGLE = 4;
		//表示する図形として、平行四辺形を選択したことを表す定数を定義
		final int CHOSE_PARALLELOGRAM = 5;
		//表示する図形の選択肢の最小数字を定数化
		...
		for (int i = 0; i < researchTimes; i++, shapeNumber++) {
			...
			//図形の種類を選択させる
			int researchChoise = returnIntegralNumber(shapeNumber + "番目の図形の種類 (1…点/2…水平直線/3…垂直直線/4…長方形/5…平行四辺形)：");
			//入力された数字が選択肢外の場合
			while (researchChoise > MAXIMUM_NUMBER || researchChoise < MINIMUM_NUMBER) {
				//選択肢内の数字を入力するように促す
				printText(MINIMUM_NUMBER + "～" + MAXIMUM_NUMBER + "の数字を入力してください。\n");
				//再度、図形の種類を選択させる
				researchChoise = returnIntegralNumber(shapeNumber + "番目の図形の種類 (1…点/2…水平直線/3…垂直直線/4…長方形/5…平行四辺形)：");
			}
			...
			case CHOSE_PARALLELOGRAM:
				//平行四辺形の幅を入力させる
				int parallelogramWidth = returnIntegralNumber("幅  ：");
				//平行四辺形の高さを入力させる
				int parallelogramHeight = returnIntegralNumber("高さ：");
				//調べる平行四辺形を生成
				storeShapes[i] = new Parallelogram(parallelogramWidth, parallelogramHeight);
				//処理を終了
				break;
			}
		}
